...
class Controllerapismpextension extends Controller
{
	...
	*/
	...
	protected function GetOrderProducts($order_id)
	{
		...
		LEFT JOIN `" . DB_PREFIX . "order_discounts` AS ord_dsc_tbl ON ord_dsc_tbl.order_id = order_product_tbl.order_id AND ord_dsc_tbl.order_product_id = order_product_tbl.order_product_id 
				...
				foreach ($product_array as $product_data) {
					...
					if ($uploading_changes === 1) { // add new or update just one image
						...
						foreach ($current_images as $current_img_desc) {
							...
							$cur_img_path = urldecode($current_img_desc['image_path']);
							if ($option_image == true) {
